window.addEventListener("DOMContentLoaded", function () { //HTML document has been completely parsed,and downloaded 
...
function playAll(songNumber) { //playAll function is responsible for playing a specific song based on its number
    var audioElement = document.getElementById("song"); //responsible for actions on audio like play and pause
    ...
    if (audioElement.getAttribute("src") == songFileName && !audioElement.paused) {
        ...
        if (songNumber === 1) {
            ...
            banner.style.backgroundImage = "url(images/b09.png)";//Can use switch instead of if else
